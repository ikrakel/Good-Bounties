...
export async function deployBountyStakeContract(bountyManagerAddress: string): Promise<BountyStakeContract> {
  const BountyStakeContract = await ethers.getContractFactory("BountyStakeContract");

  const bountyStakeContract = await BountyStakeContract.deploy(bountyManagerAddress);
