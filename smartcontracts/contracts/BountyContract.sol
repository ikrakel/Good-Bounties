...
contract BountyContract is ERC721URIStorage {
  ...
  function validateProof(
    ...
    emit ProofValidated(_bountyId, bounty.contributor, PGBountyState.VALIDATED);
  }
  ...
  function claimBounty(uint256 _bountyId) external {
    ...
    if (bounty.owner == address(0)) revert BountyDoesntExist();
    if (msg.sender != stakingContract) revert OnlyStakingContractIsAuthorized();
    if (
      bounty.state == PGBountyState.VALIDATED ||
      (bounty.state == PGBountyState.SUBMITTED &&
        block.timestamp > bounty.submittedTime + bounty.verificationPeriod)
    ) {
    ...
    emit BountyClaimed(_bountyId, bounty.contributor, PGBountyState.CLAIMED);
  }

  function expireBounty(uint256 _bountyId) external returns (Bounty memory) {
    Bounty storage bounty = idToBounties[_bountyId];

    if (bounty.owner == address(0)) revert BountyDoesntExist();
    if (bounty.submissionDeadline < block.timestamp) {
      bounty.state == PGBountyState.EXPIRED;
    }
